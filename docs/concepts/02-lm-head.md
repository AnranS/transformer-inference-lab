...
### 实测：加速是 40 倍量级，不是 1024 倍

`LMHead` 自检块（`python src/mini_transformer/lm_head.py`）在 CPU 上实测：

| 路径 | 耗时 | FLOP |
|---|---|---|
| 先算全量再切片 | ~35 ms | 79.0 GFLOP |
| 先切片再算 | ~0.8 ms | 0.077 GFLOP |

FLOP 差 1024 倍，耗时只差 **40 倍上下**（多次运行在 39～44 倍之间波动）。差距来自哪里：
...
它和 `forward(...)[:, -1, :]` 数值上完全等价，**所有 `assert_close` 断言全部通过**，
实测耗时 36.55 ms——和全量路径一样，一分计算都没省。

**这是个只有性能症状、没有功能症状的 bug。** 两条防御：
把正确顺序封进方法里（调用方只能选路径，选不了顺序），
以及在自检里量耗时而不只比数值。
